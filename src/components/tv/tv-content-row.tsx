    ...
    <section id={id} className={cn("w-full", className)} aria-label={title}>
      ...
      >
        <div className="mb-4 flex flex-wrap items-end justify-between gap-3 sm:mb-5">
          <div>
            <h2 className="zen-section-title sm:text-[22px] lg:text-[24px]">
              {title}
            </h2>
            {description ? (
              <p className="zen-body-muted mt-1 max-w-[620px] text-[14px] sm:text-[15px]">
                {description}
              </p>
            ) : null}
          </div>
      ...
      <div className="relative">
        <div
          className={cn(
            "tv-row-scroll flex snap-x snap-mandatory gap-4 overflow-x-auto pb-4 pt-1 sm:gap-5",
            bleed
              ? "pl-0 pr-0"
